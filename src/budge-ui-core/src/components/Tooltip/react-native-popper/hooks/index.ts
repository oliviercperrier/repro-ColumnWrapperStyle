...
export const usePopover = (props: { isOpen: boolean; disableAriaAttributes?: boolean }) => {
...
export const useTooltip = (props: { isOpen: boolean; disableAriaAttributes?: boolean }) => {
  const triggerId = React.useRef(generateId("trigger")).current;
  const contentId = React.useRef(generateId("content")).current;
  const { isOpen } = props;
  ...
  contentProps.accessibilityRole = Platform.OS === "web" ? "tooltip" : undefined;
...
export const useOn = (props: IOnProps) => {
  const { on = "press", onOpen } = props;

  const events = useHoverInteraction(props, {
...
const useHoverInteraction = (props: IOnProps, { enabled }: { enabled: boolean }) => {
  ...
  }, [isTriggerHovered, onOpen, enabled]);

  React.useEffect(() => {
    if (!enabled) return;
    const timeout = setTimeout(() => {
      if (!isOverlayHovered && !isTriggerHovered && !isOverlayFocused) {
        onClose();
      }
    });
    return () => {
  ...
  }, [isOverlayHovered, isTriggerHovered, isOverlayFocused, onClose, enabled]);
...
const useFocus = (targetRef: any, { enabled }: { enabled: boolean }) => {
  ...
  React.useEffect(() => {
    ...
    const currentTarget = targetRef ? targetRef.current : undefined;
    if (currentTarget) {
...
export const useAnimatedStyles = ({
  ...
  const [isExited, setIsExited] = React.useState(!isOpen);
...
export const useCloseOnOutsideClick = ({ refs, onClose, enabled }: IOutsideClickListener) => {
  React.useEffect(() => {
    if (!enabled) return;
