...
import type { IPopoverProps, IScrollContentStyle, IArrowStyles, IPopoverArrowProps, IPopoverContent } from "../types";
...
const [PopperProvider, usePopperContext] = createContext<PopperContext>("PopperContext");
...
) => <PopperProvider {...props}>{props.children}</PopperProvider>;
...
const PopperArrow = ({
  ...
  if (rest.children) {
    return <View style={[arrowStyles, resetBorderWidthStyles, rest.style]}>{rest.children}</View>;
  }
...
const getDiagonalLength = (height: number, width: number) => (height * height + width * width) ** 0.5;
